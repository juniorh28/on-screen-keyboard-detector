...
if (isSupported()) {
	...
	const unsubscribe = onOSKVisibilityChange((visibilityState) => {
		alert(`OSK ${visibilityState}`);
	...
	document
		.querySelector('#leave-be-button')
		.addEventListener('click', unsubscribe);

	document.querySelector('#recommence-button').addEventListener('click', () => {
		setTimeout(() => {
	...
	alert(
		'On-screen-keyboard detection is not supported on this Desktop Browser.'
	);
}
